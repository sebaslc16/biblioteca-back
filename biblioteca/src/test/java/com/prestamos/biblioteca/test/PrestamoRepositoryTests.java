...
class PrestamoRepositoryTests {
    ...
    @Test
    @Tag("repository")
    @DisplayName("Test del metodo findAll")
    @Order(6)
    void testFindAllPrestamos() {

        Prestamo prestamoNuevo = new Prestamo("libro1", "usuario1", 2);
        prestamoNuevo.setFechaMaximaDevolucion(Prestamo.calcularFechaMaximaDevolucion(prestamoNuevo.getTipoUsuario()));

        prestamoRepository.save(prestamoNuevo);

        List<Prestamo> listPrestamos = prestamoRepository.findAll();

        assertFalse(listPrestamos.isEmpty());
    }
}
